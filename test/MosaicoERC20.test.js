...
const MosaicoERC20 = artifacts.require("MosaicoContract");

require('truffle-test-utils').init();

require('chai').should();

contract('MosaicoContract', accounts => {
    const _name = "Mosaico";
    const _symbol = "MOS";
    const _decimals = 18;
    const _totalSupply = utils.toBN(utils.toWei("180000000", 'ether'));
    ...
    describe('Ownership', function () {
        ...
        it('should transfer ownership', async function () {
            ...
            //act
            let result = await contract.transferOwnership(newOwner);
            let ownerResult = await contract.isOwner({from: newOwner});
            //assert
            assert.web3Event(result, {
                event: 'OwnershipTransferred'
              }, 'The event is emitted'
            );
            assert.isTrue(ownerResult);   
        });

        it('should fail when not owner transfering ownership', async function () {
            //arrange
            const owner = accounts[0];
            const newOwner = accounts[1];
            //act
            try {
                await contract.transferOwnership(newOwner, {from: newOwner});
                throw "succeeded";
            }
            catch (error) {
                //assert
        ...
        it('should fail when trasfer ownership to genesis address', async function () {
            //act
            try {
                await contract.transferOwnership(_genesisAddress);
                throw "succeeded";
            }
            catch (error) {
                //assert
